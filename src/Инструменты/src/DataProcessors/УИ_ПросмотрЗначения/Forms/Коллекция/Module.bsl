...
Процедура ЗаполнитьКолонкиКоллекции(ЗначениеКоллекции)
	...
	Для Каждого ТекЭл Из ЗначениеКоллекции Цикл
		ТипЭл = ТипЗнч(ТекЭл);
		Если ТипыЭлементов.Найти(ТипЭл) = Неопределено Тогда
			ТипыЭлементов.Добавить(ТипЭл);
		КонецЕсли;
	КонецЦикла;	
	
	ОбщиеИменаСвойств = Неопределено;
	Для Каждого ТекТип Из ТипыЭлементов Цикл
		ОписаниеТипаПлатформы = УИ_ТипыПлатформыКлиентСервер.ОписаниеТипаПлатформыПоТипу(ТекТип);
		Если ОписаниеТипаПлатформы = Неопределено Тогда
			Продолжить;
		КонецЕсли; 
		
		ОбъектМД = Метаданные.НайтиПоТипу(ТекТип);
		ИменаСвойств = УИ_ОбщегоНазначения.ИменаСвойствОписанияТипаПлатформы(ОписаниеТипаПлатформы, Неопределено, ОбъектМД);
