...
Процедура ИмпортироватьДанныеИмпортаНаСервере(ДанныеИмпорта)
	Если ДанныеИмпорта.Формат.Идентификатор = "Инсомния4" Тогда
		ИмпортироватьДанныеИнсомнии4(ДанныеИмпорта);
	ИначеЕсли ДанныеИмпорта.Формат.Идентификатор = "curl" Тогда 
		ИмпортироватьДанныеCurl(ДанныеИмпорта);
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Функция Инсомния4ПрочитатьРабочиеОбласти(СодержимоеФайла)
...
&НаСервере
Процедура ИмпортироватьДанныеИнсомнии4(ДанныеИмпорта)
	...
	Попытка
		СодержимоеФайла = УИ_ОбщегоНазначенияКлиентСервер.мПрочитатьJSON(ТекстовыйДокумент.ПолучитьТекст());//Структура
	Исключение
		УИ_ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Содержимое файла не является JSON");
		Возврат;
	КонецПопытки;
	...
	НаборыЗначений = Новый Структура;

	Для Каждого ТекСтр Из СодержимоеФайла.resources Цикл
		Если ТекСтр._type = "workspace" Тогда
			Области.Вставить(ТекСтр._id, ТекСтр);
		ИначеЕсли ТекСтр._type = "request_group" Тогда
			Группы.Вставить(ТекСтр._id, ТекСтр);
		ИначеЕсли ТекСтр._type = "request" Тогда
			Запросы.Вставить(ТекСтр._id, ТекСтр);
		ИначеЕсли ТекСтр._type = "environment" Тогда
			НаборыЗначений.Вставить(ТекСтр._id, ТекСтр);
		КонецЕсли;	
	КонецЦикла;
...
&НаСервере
Процедура ИмпортироватьДанныеCurl(ДанныеИмпорта)
...
Функция ПараметрыВыбораНастроекИмпорта()
	...
	НовыйФормат.Представление = "Инсомния (v4)";
