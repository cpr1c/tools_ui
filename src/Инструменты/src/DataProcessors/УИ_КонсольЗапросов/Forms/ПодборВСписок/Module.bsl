...
Процедура ЗаполнитьСписокИзФайловНаСервере(АдресХранилища, Расширение)
	...
	Для Каждого ТекФайл Из МассивФайлов Цикл
		...
		ИначеЕсли Расширение = "xml" Тогда
			СписокЗнач = ОбщегоНазначения.ЗначениеИзСтрокиXML(ТекстИзФайла);
...
Процедура КомандаПередатьВСтрокуXML(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("КомандаПередатьВСтрокуВнутрЗавершение", 
												  ЭтотОбъект,
...
&НаКлиенте
Процедура КомандаПередатьВСтрокуВнутрЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Функция ПолучитьФайлСпискаЗначенийВСтрокуВнутр()
	СтрокаXML = ОбщегоНазначения.ЗначениеВСтрокуXML(СписокЭлементов);
