...
// У и подготовленный URL.
// 
// Параметры:
//   URL - Строка - URL ресурса, к которому будет отправлен запрос.
//   ПараметрыЗапроса - Структура, Соответствие Из КлючИЗначение- параметры, которые будут отправлены в URL (часть после ?):
//     * Ключ - Строка - ключ параметра в URL.
//     * Значение - Строка - значение параметра URL
//                  - Массив из Строка - сформирует строку из нескольких параметров: key=value1&key=value2 и т.д.
// 
// Возвращаемое значение:
// Строка 
Функция УИ_ПодготовленныйURL(URL, ПараметрыЗапроса = Неопределено) Экспорт
	Возврат ПодготовитьURL(URL, ПараметрыЗапроса);
КонецФункции

// У и собрать адрес ресурса.
// 
// Параметры:
//  СтруктураURL - см. РазобратьURL
// Возвращаемое значение:
//  Строка - У и собрать адрес ресурса
Функция УИ_СобратьАдресРесурса(СтруктураURL) Экспорт
	Возврат СобратьАдресРесурса(СтруктураURL, Новый Структура);
КонецФункции
