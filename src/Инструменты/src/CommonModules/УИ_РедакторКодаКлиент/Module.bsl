...
Функция ВсеРедакторыФормыИнициализированы(РедакторыФормы)
	...
	Для Каждого КлючЗначение Из РедакторыФормы Цикл
		Если Не КлючЗначение.Значение.Инициализирован
			И КлючЗначение.Значение.Видимость Тогда
			Результат = Ложь;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура РедакторКодаОтложеннаяИнициализацияРедакторов(Форма) Экспорт
	...
	Если Не УИ_РедакторКодаКлиентСервер.ПервичнаяИнициализацияРедакторовПрошла(Форма) Тогда
		Форма.Подключаемый_РедакторКодаЗавершениеИнициализации();
		УИ_РедакторКодаКлиентСервер.УстановитьПризнакПервичнаяИнициализацияРедакторовПрошла(Форма, Истина);
	КонецЕсли;

	Если ВидРедактора = ВидыРедактора.Monaco Тогда
		НачатьЗагрузкуШаблоновКодаВРедакторы(Форма, РедакторыФормы);
	КонецЕсли;
КонецПроцедуры
...
Процедура НачатьЗагрузкуШаблоновКодаВРедакторыЗавершение(Результат, ДопПараметры) Экспорт
	...
	ВидыРедактора = УИ_РедакторКодаКлиентСервер.ВариантыРедактораКода();
	РедакторыФормы = ДопПараметры.Форма[УИ_РедакторКодаКлиентСервер.ИмяРеквизитаРедактораКодаСписокРедакторовФормы()];
	
	Если ВидРедактора<>ВидыРедактора.Monaco Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого КлючЗначениеРедактора Из РедакторыФормы Цикл
		ПараметрыРедактора = КлючЗначениеРедактора.Значение;
		Если Не ПараметрыРедактора.Инициализирован Тогда
			Возврат;
		КонецЕсли;
		ЭлементФормыРедактора = ДопПараметры.Форма.Элементы[ПараметрыРедактора.ПолеРедактора];
...
Процедура НачатьЗагрузкуШаблоновКодаВРедакторы(Форма, РедакторыФормы)
	ПараметрыРедактора = Неопределено;
	Для Каждого КлючЗначение Из РедакторыФормы Цикл
		ПараметрыРедактора = КлючЗначение.Значение.ПараметрыРедактора;
		Прервать;
	КонецЦикла;
	...
	ДопПараметры.Вставить("ОписаниеОповещенияОЗавершении",
		Новый ОписаниеОповещения("НачатьЗагрузкуШаблоновКодаВРедакторыЗавершение", ЭтотОбъект, Новый Структура("Форма",
		Форма)));
