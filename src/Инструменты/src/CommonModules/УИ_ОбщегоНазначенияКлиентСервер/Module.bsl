...
//		Возврат СтрЗаменить(ИдентификаторТипа, ":", "_");
		...
		Возврат ЧастиИдентификатора[ЧастиИдентификатора.Количество() - 1];
//
...
Процедура УстановитьЗначениеВЭлементыФормыКонтейнераУниверсальногоЗначения(Форма, ДанныеФормы, Значение, УстановитьЗначение) Экспорт
	...
	Если ОписаниеТипа = Неопределено Тогда
		ТипЗначения = ТипЗнч(Значение);
		СообщитьПользователю("Не удалось получить описание типа "
							 + ТипЗначения
...
#КонецЕсли
	ИначеЕсли Хранилище.Вид = ВидыКонтейнераЗначения.ВременноеХранилище Тогда
