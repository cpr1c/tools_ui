...
Процедура НачатьЗапускПриложенияСПодключениемРасширенияРаботыСФайлами(ОписаниеОповещения, СтрокаКоманды, ТекущийКаталог = Неопределено, ДождатьсяЗавершения = Ложь) Экспорт
	...
	ПодключитьРасширениеРаботыСФайламиСВозможнойУстановкой(Новый ОписаниеОповещения("НачатьЗапускПриложенияСПодключениемРасширенияРаботыСФайламиЗавершениеПодключенияРасширенияРаботыСФайлами", ЭтотОбъект, ПараметрыОповещения));	
КонецПроцедуры
...
//  	* ТекущийКаталог - Неопределено, Строка - Задает текущий каталог запускаемого приложения. В режиме веб-клиента игнорируется.
//  	* ДождатьсяЗавершения - Булево - дожидаться завершения запущенного приложения перед продолжением работы
Процедура НачатьЗапускПриложенияСПодключениемРасширенияРаботыСФайламиЗавершениеПодключенияРасширенияРаботыСФайлами(Результат, ДополнительныеПараметры) Экспорт
	Если Не Результат Тогда
		Возврат;
	КонецЕсли;
	
	НачатьЗапускПриложения(ДополнительныеПараметры.ОписаниеОповещения, ДополнительныеПараметры.СтрокаКоманды, ДополнительныеПараметры.ТекущийКаталог, ДополнительныеПараметры.ДождатьсяЗавершения);
КонецПроцедуры
...
Процедура ПросмотретьЗначение(КонтейнерЗначения, Форма, Выражение, ВариантФормы = Неопределено, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	ИначеЕсли ВариантФормы = ВариантыФормы.Коллекция Тогда
		ИмяФормы = "Коллекция";
	Иначе
		Если КонтейнерЗначения.ОписаниеТипа.ЭтоКоллекция И КонтейнерЗначения.ОписаниеТипа.Свойства.Количество() = 0 Тогда
			ИмяФормы = "Коллекция";
		Иначе
			ИмяФормы = "Форма";
		КонецЕсли;
...
Процедура НачатьЧтениеФайлаЗавершениеПомещенияФайлов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		...
		ОписаниеФайла.Вставить("Хранение", ПомещенныйФайл.Хранение);
		Если УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.13") Тогда
			ОписаниеФайла.Вставить("ПолноеИмя", ПомещенныйФайл.ПолноеИмя);
		Иначе
			ОписаниеФайла.Вставить("ПолноеИмя", ПомещенныйФайл.Имя);
		КонецЕсли;
