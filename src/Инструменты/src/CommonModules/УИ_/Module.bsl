...
#Область ВыполнитьКодВОтладчике
// Описание
...
// МассивИменПараметров - Строка -  Разделенные запятыми имена параметров
// Возвращаемое значение:
// Произвольный -  Параметр_Р - Результат выполнения сохранения алгоритма
//@skip-check method-too-many-params
Функция _ВыполниитьКодВОтладчике(Код, Параметр1 = Неопределено, Параметр2 = Неопределено,  Параметр3 = Неопределено, Параметр4 = Неопределено, Параметр5 = Неопределено)  Экспорт
...
// Параметры:
// А - Произвольный - Произвольное значение
// Б - Произвольный - Произвольное значение
// Возвращаемое значение:
// А - Результат выполнения сохранения алгоритма

Функция Присвоить(Параметр_А,Параметр_Б) Экспорт
Параметр_А=Параметр_Б; 
Возврат Параметр_А;
КонецФункции

// Описание
// Можно смотреть переменные на разных уровнях стека
// Если поставить в этой функцию точку останова, то 
// можно увидеть что на разных уровнях стека переменная А принимает разные значения
// Параметры:
// Параметр_А - Произвольный - Произвольное значение
// Возвращаемое значение:
// Параметр_А - Результат выполнения сохранения алгоритма

Функция Тест(Параметр_А)
    Если Параметр_А>10 Тогда
        Возврат ложь;
    Иначе
        Возврат Тест(Параметр_А+1);
    КонецЕсли;
КонецФункции
