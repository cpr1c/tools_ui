...
Процедура ФормаПриСозданииНаСервере(Форма, ВидРедактора = Неопределено) Экспорт
	...
	Если ВидРедактора = ВариантыРедактора.Ace Тогда
		Форма[ИмяРеквизитаДанныеБиблиотекРедакторов].Вставить(КлючДанныхБиблиотекиВзаимодействия,
															  ДанныеБиблиотекиОбщегоМакета("УИ_AceColaborator",
...
Процедура СоздатьЭлементыРедактораКода(Форма, ИдентификаторРедактора, ПолеРедактора, СобытияРедактора = Неопределено,
	...
	Если ВидРедактора = ВариантыРедактора.Ace Тогда
		...
		Форма[ДанныеРедактора.ИмяРеквизита] = ТекстПоляHTMLРедактораAce(ДанныеБиблиотекРедакторов[ВариантыРедактора.Ace]);
	КонецЕсли;
...
Функция ДанныеБиблиотекиОбщегоМакета(ИмяМакета, ИдентификаторФормы) Экспорт
	...
	Для Каждого ФайлБиблиотеки Из ФайлыАрхива Цикл
		...
		Если НРег(ФайлБиблиотеки.Расширение) = ".js" Тогда
			...
			ДанныеБиблиотеки.Скрипты.Добавить(ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайла, ИдентификаторФормы));
		ИначеЕсли НРег(ФайлБиблиотеки.Расширение) = ".css" Тогда
			Текст = Новый ТекстовыйДокумент();
			Текст.Прочитать(ФайлБиблиотеки.ПолноеИмя);
			ДанныеБиблиотеки.Стили.Добавить(Текст.ПолучитьТекст());
		КонецЕсли;
	КонецЦикла;
...
//  ЭтоWindowsКлиент -Булево-Это windows клиент
//  ЭтоВебКлиент -Булево-Это веб клиент
//  ВидРедактора - Строка, Неопределено -  Вид редактора
// 
// Возвращаемое значение:
//  Неопределено -  Данные библиотеки редактора
// Возвращаемое значение:
//  Строка -  Адрес бибиотеки во временном хранилище
// Возвращаемое значение:
//  см. УИ_РедакторКодаКлиентСервер.НовыйДанныеБиблиотекиРедактора
Функция ДанныеБиблиотекиРедактора(ИдентификаторФормы, ЭтоWindowsКлиент, ЭтоВебКлиент, ВидРедактора = Неопределено)
	Если ВидРедактора = Неопределено Тогда
	...
	Если ВидРедактора <> ВариантыРедактора.Ace Тогда
		Возврат ПоместитьБиблиотекуВоВременноеХранилище(ИдентификаторФормы,
														ЭтоWindowsКлиент,
														ЭтоВебКлиент,
														ВидРедактора);
	КонецЕсли; 
	
	Возврат ДанныеБиблиотекиОбщегоМакета("УИ_Ace", ИдентификаторФормы);

КонецФункции

Функция ПоместитьБиблиотекуВоВременноеХранилище(ИдентификаторФормы, ЭтоWindowsКлиент, ЭтоВебКлиент,
	ВидРедактора = Неопределено) 
	Если ВидРедактора = Неопределено Тогда
		ВидРедактора = ТекущийВариантРедактораКода1С();
	КонецЕсли;
	ВариантыРедактора = УИ_РедакторКодаКлиентСервер.ВариантыРедактораКода();
	...
	Если ВидРедактора = ВариантыРедактора.Monaco Тогда
		...
		ИначеЕсли ЭтоWindowsКлиент Или УИ_ОбщегоНазначенияКлиентСервер.ВерсияПлатформыНеМладше("8.3.24") Тогда
